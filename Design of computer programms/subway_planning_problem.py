...
def subway(**lines):
    ...
    for line_name in lines:
        stations=lines[line_name].split()
        number=len(stations)
        for i in range(number):
            if stations[i] not in rst:
                neighbor={}
            else:
                neighbor=rst[stations[i]]
            
            if i>=1:
                neighbor[stations[i-1]]=line_name
            if i<number-1:
                neighbor[stations[i+1]]=line_name
            
            rst[stations[i]]=neighbor
...
def ride(here, there, system=boston):
    ...
    def is_goal(station): return station==there
    ...
    for station1 in system:
        for station2 in system:
            if station1 !=station2:
                path=ride(station1,station2)
                if path_length(path)>max_path_length:
                    longest_ride=path
                    max_path_length=path_length(path)
    
    return longest_ride
                
def path_length(path): return len(path_states(path))
