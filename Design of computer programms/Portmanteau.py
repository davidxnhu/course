...
"""
def prefixes(words):
    "Find all prefixes in the word list"
    results=set()
    for word in words:
...
def suffixes(words):
    "Find all suffixes in the word list"
    results=set()
    for word in words:
...
def natalie(words):
    ...
    for a in SUFFIXES:
        for b in PREFIXES:
            if a==b and a!='':  # the common part is the mid section
                mid=a
                start=find_prefix(words,a)
                end=find_suffix(words,b)
                for s in start:
                    for e in end:
                        if s+mid != mid+e:
                            result.add((s,mid,e))
    ...
    if result:
        start,mid,end=max(result,key=score)
        return start+mid+end
    else:
        return None

def find_prefix(words,suffix):
    "Find all possible prefixes in the words with fixed suffix"
    result=set()
    length=len(suffix)
    for word in words:
        if len(word)-length>=1 and word[len(word)-length:]==suffix:
            result.add(word[:len(word)-length])
    return result


def find_suffix(words,prefix):
    "Find all possible suffixes in the words with fixed prefix"
    result=set()
    length=len(prefix)
    for word in words:
        if len(word)-length>=1 and word[:length]==prefix:
            result.add(word[length:])
    return result

def score((start,mid,end)):
    total_length=len(start+mid+end)
    return total_length-abs(len(start)-total_length/4.)-abs(len(mid)-total_length/2.)-abs(len(end)-total_length/4.)
