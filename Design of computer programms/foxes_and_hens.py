...
def do(action, state):
    ...
    number=len(cards)
    i=random.randint(0,number-1) if number>=2 else 0
    
    next_cards=cards[:i]
    if i!=number-1: next_cards+=cards[i+1:]
    
    if action=='gather':
        return (score+yard,0,next_cards)
    else:
        return (score,yard+1,next_cards) if cards[i]=='H'else (score,0,next_cards)
...
def strategy(state):
    (score, yard, cards) = state
    # your code here
    return best_action(state,actions,Q,U)


def actions(state):
    _,yard,_=state
    return ['gather','wait'] if yard else ['wait']
...
def U(state):
    (score,yard,cards)=state
    if cards.count('F')==0: return score+yard+len(cards)
    elif cards.count('H')==0: return score+yard
    else:
        return max(Q(state,action,U) for action in actions(state))
    

def Q(state,action,U):
    (score,yard,cards)=state
    number=len(cards)
    no_of_fox=cards.count('F')
    no_of_hen=cards.count('H')

    less_1_fox=''
    if no_of_fox>1:
        less_1_fox+='F'*(no_of_fox-1)
    less_1_fox+='H'*no_of_hen

    less_1_hen='F'*no_of_fox
    if no_of_hen>1:
        less_1_hen+='H'*(no_of_hen-1)
    
    
    if action=='gather':
        return U((score+yard,0,less_1_fox))*float(no_of_fox)/number+U((score+yard,0,less_1_hen))*float(no_of_hen)/number
    
    if action=='wait':
        return U((score,0,less_1_fox))*float(no_of_fox)/number+U((score,yard+1,less_1_hen))*float(no_of_hen)/number
