...
def boggle_words(board, minlength=3):
    ...
    # your code here
    results=set()
    return helper(board,'',minlength,results,[x for x in range(len(board))])


def helper(board,pre,minlength,results,ranges):
    if pre in WORDS and len(pre)>=minlength: 
        results.add(pre)
    if pre in PREFIXES:
        for i in ranges:
            if i in range(len(board)) and is_letter(board[i]):
                temp=board[i]
                next_neighbors=list(neighbors(i,size(board)))
                helper(replace(board,i,'.'),pre+temp,minlength,results,next_neighbors)
    return results    
...
def is_letter(L): return L in LETTERS
def test():
