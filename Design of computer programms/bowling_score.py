...
def bowling(balls):
    ...
    score,i,frame=0,0,1   # score starts from 0, balls starts from 0,frame starts from 1
    while frame<=9:
        if balls[i]==10:  # if it's a strike
            score+=(10+balls[i+1]+balls[i+2])
            i+=1
            frame+=1
        elif balls[i]+balls[i+1]==10:     # if it's a spare
            score+=(10+balls[i+2])
            i+=2
            frame+=1
        else:
